...
def set_args():
    ...
    parser.add_argument('--test_path', default='/LLM/Abirate/spo_1.json', type=str, help='')
    parser.add_argument('--device', default='1', type=str, help='')
    parser.add_argument('--ori_model_dir',
                        default="/LLM/baichuan-inc/Baichuan2-7B-Chat/", type=str,
                        help='')
    parser.add_argument('--model_dir',
                        default="/LLM/baichuan/19baichuan-13B-lora_-mlp_qa-output15/", type=str,help='')
    ...
    with open(args.test_path, "r", encoding="utf-8") as fh:
        for i, line in enumerate(tqdm(fh, desc="iter")):
            with torch.no_grad():
                ...
                print("lora_answer:" + str(pre_res[1:]))
                ...
                f1 += f
                save_data.append(
                    {"text": sample["input"], "ori_answer": sample["output"], "gen_answer": pre_res[1:], "f1": f})
