...
class LoraConfig(PeftConfig):
    ...
    Args:
        ...
        modules_to_save (`List[str]`):List of modules apart from LoRA layers to be set as trainable
            elf.lora_And saved in the final checkpoint.
...
class LoraLayer(BaseTunerLayer):
    def __init__(self, in_features: int, out_features: int, **kwargs):
        ...
        self.mid_rank = int(in_features*0.25)
        #self.mid_rank = 192
        if self.mid_rank <2:
           self.mid_rank=int(2)
        self.kwargs = kwargs
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        if r > 0:
            #self.mid_rank = self.out_features/4
            self.lora_A.update(nn.ModuleDict({adapter_name: nn.Linear(self.in_features, r, bias=False)}))
            #self.lora_B.update(nn.ModuleDict({adapter_name: nn.Linear(r,r, bias=False)}))
            self.lora_B.update(nn.ModuleDict({adapter_name: nn.Linear(r, self.mid_rank, bias=False)}))
            self.lora_C.update(nn.ModuleDict({adapter_name: nn.Linear(self.mid_rank, r, bias=False)}))
            self.lora_D.update(nn.ModuleDict({adapter_name: nn.Linear(r, self.out_features, bias=False)}))
            self.lora_E.update(nn.ModuleDict({adapter_name: nn.Linear(r, r, bias=False)}))
    ...
    def reset_lora_parameters(self, adapter_name):
        if adapter_name in self.lora_A.keys():
            ...
            #nn.init.zeros_(self.lora_E[adapter_name].weight)
            nn.init.kaiming_uniform_(self.lora_C[adapter_name].weight, a=math.sqrt(5))
            nn.init.kaiming_uniform_(self.lora_E[adapter_name].weight, a=math.sqrt(5))
            #nn.init.kaiming_uniform_(self.lora_D[adapter_name].weight, a=math.sqrt(5))            
...
class Linear(nn.Linear, LoraLayer):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            # 应用dropout操作到输入数据x
            dropout_result = self.lora_dropout[self.active_adapter](x)

            # 通过self.lora_A进行特定适配器操作
            lora_A_result = self.lora_A[self.active_adapter](dropout_result)

            # 通过self.lora_B进一步处理
            lora_B_result = self.lora_B[self.active_adapter](lora_A_result)
            lora_E_result = self.lora_E[self.active_adapter](lora_A_result)
            # 通过self.lora_C进行处理
            lora_C_result = self.lora_C[self.active_adapter](lora_B_result)  # 注意：lora_B的结果作为lora_C的输入

            # 将lora_A的结果与lora_C的结果融合
            merged_result = lora_E_result + lora_C_result+lora_A_result

            # 通过self.lora_D进一步处理
            lora_D_result = self.lora_D[self.active_adapter](merged_result)

            # 缩放最终的结果
            scaled_result = lora_D_result * self.scaling[self.active_adapter]
   ...
   #         dropout_result = self.lora_dropout[self.active_adapter](x)
         ...
         #   lora_D_result = self.lora_D[self.active_adapter](merged_result)
          ...
          #  scaled_result = lora_D_result * self.scaling[self.active_adapter]
           ...
           # result += scaled_result
            ...
            # 将lora_A的结果与lora_C的结果融合
                    merged_result = lora_A_result + lora_C_result+ lora_E_result
    ...
    if is_bnb_4bit_available():
        ...
        class Linear4bit(bnb.nn.Linear4bit, LoraLayer):
            ...
            # 缩放最终的结果
                        output = lora_D_result * self.scaling[self.active_adapter]
