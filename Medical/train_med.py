...
def find_all_linear_names_linear(model):
    """
    找出所有全连接层，为所有全连接添加adapter
    """
    cls = bnb.nn.Linear
...
def main():
    ...
    raw_datasets = load_dataset(
        ...
        # True` is required when you want to use a private model.
        # use_auth_token=True if model_args.use_auth_token else None,
    )

    prefix = "If you are a doctor, please answer the medical questions based on the patient's description."
    med_prefix = "If you are a doctor, please answer the medical questions based on the patient's description."
    ...
    if model_name == "Baichuan2-7B-Chat" or model_name == "Baichuan2-13B-Chat" or model_name == "Qwen-7B-Chat":
 ...
 #                                                        bnb_4bit_compute_dtype=torch_dtype,
  #                                                       bnb_4bit_use_double_quant=True,
   #                                                      bnb_4bit_quant_type="nf4",
    ...
    target_modules = ['up_proj', 'gate_proj', 'W_pack', 'down_proj', 'o_proj']
    ...
    if model_name == "Baichuan2-7B-Chat" or model_name == "Baichuan2-13B-Chat":
        # model.supports_gradient_checkpointing = True  #节约cuda
        # model.gradient_checkpointing_enable()   make cuda reduce but add time of train
...
#    )
    ...
    # Get the column names for input/target. text/completion
    prompt_column = data_args.prompt_column
    response_column = data_args.response_column
    history_column = data_args.history_column
    print("--------gradient_checkpointing------------"+str(training_args.gradient_checkpointing))
    ...
    train_data_process_dict = {
        ...
        "Baichuan2-13B-Chat": preprocess_function_train_bai_chuan,
        "Baichuan2-7B-Chat": preprocess_function_train_bai_chuan,
    ...
    # if text not in column_names set text 0 dis
    # text_column_name = "text" if "text" in column_names else column_names[0]
    print_dataset_example(train_dataset[0])

    # Data collator
    label_pad_token_id = -100 if data_args.ignore_pad_token_for_loss else tokenizer.pad_token_id
    # make datacollator for random mask
